    ...
    public String countOfAtoms(String formula) {
        ...
        for (int i = 0; i < len; i++) {
            if (formula.charAt(i) == '(') {
                ...
                final int multiFinal = !countStr.equals("") ? Integer.parseInt(countStr) : 1;
                map.forEach((k, v) -> {
                    top.put(k, top.getOrDefault(k, 0) + v * multiFinal);
                });
                ...
                if (Character.isUpperCase(formula.charAt(i))) {
                    ...
                    while (i + 1 < len && Character.isLowerCase(formula.charAt(i + 1))) {
                        name.append(formula.charAt(++i));
                    }
                    String atomsName = name.toString();
                    StringBuilder countBuilder = new StringBuilder();
                    while (i + 1 < len && Character.isDigit(formula.charAt(i + 1))) {
                        countBuilder.append(formula.charAt(++i));
                    }
                    String countStr = countBuilder.toString();
                    int multi = !countStr.equals("") ? Integer.parseInt(countStr) : 1;
                    stack.peek().put(atomsName, stack.peek().getOrDefault(atomsName, 0) + multi);
                }
